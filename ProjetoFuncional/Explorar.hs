...
coletarLocal status itens idLocal = coletarLocal' status itens idLocal eventos
...
coletarLocal' status itens idLocal (evento:tail)
  ...
  where personagem = fst status
        ...
        mensagem = diario personagem ++ "adquiriu item " ++ nome itemColeta ++ "/n" 
...
investigarLocal status itens idLocal = investigarLocal' status itens idLocal eventos
